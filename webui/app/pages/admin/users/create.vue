<template>
  <NuxtLayout name="admin">
    <div class="flex items-center justify-between px-6 py-4 bg-white dark:bg-slate-800 border-b dark:border-slate-700">
      <h1 class="text-lg font-semibold text-slate-800 dark:text-white capitalize">Tambah User Baru</h1>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-6">
            ...
            <div class="flex flex-col items-center gap-4">
              <div class="relative group">
                <img 
                  :src="previewUrl || 'https://ui-avatars.com/api/?name=New+User&background=0D8ABC&color=fff'" 
...
onMounted(async () => {
  try {
    const [rolesRes, provRes] = await Promise.all([
      get('/admin/roles'),
      fetchProvinsi()
    ])
