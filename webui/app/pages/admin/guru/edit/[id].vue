...
onMounted(async () => {
  try {
    const [provRes, mgmpRes, sekolahRes, teacherRes] = await Promise.all([
      fetchProvinsi(),
      get('/admin/mgmp-list'),
      get('/admin/sekolah-list'),
      get(`/admin/user/guru/${route.params.id}`)
    ])
    
    provinsis.value = provRes
    mgmps.value = mgmpRes
    sekolahs.value = sekolahRes
    
    // Populate form with teacher data
    const guru = teacherRes.data
    console.log('Editing teacher:', guru)
    ...
    if (guru.sekolah && guru.sekolah.length > 0) {
        form.sekolah_id = guru.sekolah[0].id
        form.status_guru = guru.sekolah[0].pivot?.status || 'aktif'
    }

    // Load dependent wilayah data sequentially to avoid race conditions
    if (form.provinsi_id) {
        console.log('Loading kotas for provinsi:', form.provinsi_id)
        kotas.value = await fetchKota(form.provinsi_id)
    }
    if (form.kota_id) {
        console.log('Loading kecamatans for kota:', form.kota_id)
        kecamatans.value = await fetchKecamatan(form.kota_id)
    }
    if (form.kecamatan_id) {
        console.log('Loading kelurahans for kecamatan:', form.kecamatan_id)
        kelurahans.value = await fetchKelurahan(form.kecamatan_id)
    }

  } catch (error) {
    console.error('Failed to load initial data:', error)
    showAlert({ title: 'Gagal!', text: 'Data guru tidak ditemukan atau terjadi kesalahan server.', icon: 'error' })
...
const submitForm = async () => {
  ...
  try {
    ...
    formData.append('_method', 'PUT') // Spoofing for Laravel Multipart PUT
    ...
    await fetchapi(`/admin/user/guru/${route.params.id}`, {
