<template>
  <NuxtLayout name="admin">
    <div class="flex items-center justify-between px-6 py-4 bg-white dark:bg-slate-800 border-b dark:border-slate-700">
      <h1 class="text-lg font-semibold text-slate-800 dark:text-white capitalize">Tambah Guru Baru</h1>
      <nav class="flex items-center space-x-2 text-sm">
        ...
        <NuxtLink to="/admin/guru" class="text-blue-500 hover:underline">Guru</NuxtLink>
        <Icon name="ph:caret-right-bold" class="text-slate-400" size="12" />
    ...
    <div class="p-6">
      <form @submit.prevent="submitForm" class="grid grid-cols-12 gap-6">
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-6">
            ...
            <div class="space-y-4">
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <input v-model="form.password" type="password" placeholder="********" class="form-input-custom" required />
              ...
              <div class="flex flex-col gap-1.5">
                ...
                <input v-model="form.password_confirmation" type="password" placeholder="********" class="form-input-custom" required />
              </div>
            </div>
          </div>
          ...
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-6">
            ...
            <div class="flex flex-col items-center gap-4">
              <div class="relative group">
                <img 
                  :src="previewUrl || 'https://ui-avatars.com/api/?name=New+Guru&background=0D8ABC&color=fff'" 
        ...
        <div class="col-span-12 lg:col-span-8 space-y-6">
          ...
          <!-- Data Profesi Guru Section -->
          <div class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border dark:border-slate-700 p-6">
            <h3 class="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider mb-6 pb-2 border-b dark:border-slate-700 flex items-center gap-2">
              <Icon name="ph:briefcase-bold" /> Data Profesi & Sekolah
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="flex flex-col gap-1.5">
              ...
              <div class="flex flex-col gap-1.5">
                <label class="text-xs font-bold text-slate-500 uppercase">Sekolah Induk</label>
                <select v-model="form.sekolah_id" class="form-input-custom" required>
                  <option value="">Pilih Sekolah</option>
                  <option v-for="item in sekolahs" :key="item.id" :value="item.id">{{ item.nama_sekolah }}</option>
                </select>
              </div>
              <div class="flex flex-col gap-1.5">
            ...
            <div class="mt-8 pt-6 border-t dark:border-slate-700 flex justify-end gap-3">
              <NuxtLink to="/admin/guru" class="px-6 py-2 rounded-lg text-sm font-semibold border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                Batal
              </NuxtLink>
              <button 
                type="submit" 
...
onMounted(async () => {
  // Clear cached wilayah for fresh start if needed
  // localStorage.removeItem('list_provinsi') 
  
  try {
    provinsis.value = await fetchProvinsi()
    console.log('Provinsis loaded:', provinsis.value.length)
  } catch (e) {
    console.error('Failed to load provinces:', e)
  }

  try {
    mgmps.value = await get('/admin/mgmp-list')
  } catch (e) {
    console.error('Failed to load MGMP list:', e)
  }

  try {
    sekolahs.value = await get('/admin/sekolah-list')
  } catch (e) {
...
const submitForm = async () => {
  ...
  try {
    ...
    await fetchapi('/admin/user/guru', {
      method: 'POST',
      body: formData,
      headers: {
    ...
    showAlert({
      title: 'Berhasil!',
      text: 'Data guru baru telah ditambahkan.',
      icon: 'success',
      timer: 1500
    })
